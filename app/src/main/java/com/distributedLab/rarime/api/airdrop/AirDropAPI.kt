...
interface AirDropAPI {
    ...
    suspend fun createAirDrop(@Body body: CreateAirDropBody): Response<AirDropResponseBody>

    @GET("${BaseConfig.RELAYER_URL}/integrations/airdrop-svc/airdrops/{nullifier}")
    suspend fun getAirDropByNullifier(@Path("nullifier") nullifier: String): Response<AirDropResponseBody>

    @GET("${BaseConfig.RELAYER_URL}/integrations/airdrop-svc/airdrops/params")
    suspend fun getAirDropParams(): Response<AirdropEventParamsBody>
}